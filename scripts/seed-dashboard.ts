...
// Inline schemas (avoids path alias issues in tsx)
...
async function seed() {
  ...
  const ImagingStudy = mongoose.models.ImagingStudy || mongoose.model('ImagingStudy', ImagingStudySchema);
  const PharmacyItem = mongoose.models.PharmacyItem || mongoose.model('PharmacyItem', PharmacyItemSchema);
  const User = mongoose.model('User', new mongoose.Schema({ email: String }));

  // Get admin & doctor user IDs
  const admin = await User.findOne({ email: 'admin@medicore.com' });
  const doctor = await User.findOne({ email: 'doctor@medicore.com' });
  if (!admin || !doctor) { console.error('Run npm run seed first!'); process.exit(1); }
  ...
  await MedicalRecord.insertMany(
    patients.slice(0, 5).map((p, i) => ({
      ...
      patient: p._id, doctor: doctor._id, type: 'consultation',
